...
#speaker_hash was used in order to avoid a two dimensional array declaration
def set_speaker_hash(speakers)
	speaker_hash = Hash.new

	speakers.each do |speaker|
		#ignore the speech tag that contains the ALL text within the SPEAKER tag
		if speaker.text == "ALL"
			next
		end
		#sets the speaker key value to 0 for further additions
		speaker_hash[speaker.text] = 0
	end
	return speaker_hash
end
...
#grabs all the speaker tags
speakers = xml_file.css("SPEAKER")

speaker_hash = set_speaker_hash(speakers)
